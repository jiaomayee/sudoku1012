          ...
          for (const common of commonCandidates) {
            ...
            for (let row = 0; row < 9; row++) {
              for (let col = 0; col < 9; col++) {
                ...
                console.log(`      Checking target cell (${row},${col}): sharesWithCell1=${sharesWithCell1}, sharesWithCell2=${sharesWithCell2}`);
...
// 特别检查目标单元格(5,4)是否满足条件
console.log("\n=== 特别检查目标单元格(5,4) ===");
const targetCell = { row: 5, col: 4 };
const xzCell = { row: 3, col: 5 };
const yzCell = { row: 6, col: 4 };

const checkAreInSameUnit = (cell1, cell2) => {
  // 共享行
  if (cell1.row === cell2.row) return true;
  // 共享列
  if (cell1.col === cell2.col) return true;
...
console.log(`目标单元格(5,4)与XZ单元格(3,5)在同一单元: ${checkAreInSameUnit(targetCell, xzCell)}`);
console.log(`目标单元格(5,4)与YZ单元格(6,4)在同一单元: ${checkAreInSameUnit(targetCell, yzCell)}`);

// 检查目标单元格的候选数
const targetNotes = testPencilNotes["5-4"] || [];
console.log(`目标单元格(5,4)的候选数: [${targetNotes.join(', ')}]`);

// 检查公共候选数9是否在目标单元格中
const commonCandidate = 9;
console.log(`公共候选数${commonCandidate}在目标单元格中: ${targetNotes.includes(commonCandidate)}`);